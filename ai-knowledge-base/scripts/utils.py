...
def list_wiki_articles() -> list[Path]:
    """List all wiki article files across concepts, connections, sources, entities, qa."""
    articles = []
    for subdir in ARTICLE_DIRS:
        if subdir.exists():
...
def parse_frontmatter(content: str) -> dict | None:
    """Parse a minimal YAML frontmatter block from a markdown article.

    Returns a dict of string keys to string/list values, or None if no
    frontmatter block is present. Only supports the flat schema used by the
    knowledge base (scalar values and simple `[a, b]` lists).
    """
