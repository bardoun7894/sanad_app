...
## Unified frontmatter schema (use this exactly — supersedes AGENTS.md if they conflict)

Every article you create or update MUST start with this YAML frontmatter block:
...
When updating an existing article, append the current log to the `sources:` list (do not
remove existing entries) and bump `updated:` to today. Leave `created:` alone.

## Schema (AGENTS.md, for structure of the body below the frontmatter)
...
3. **Reuse existing source pages and entity pages.** This knowledge base also holds two other
   article types populated by manual document ingestion:
   - `knowledge/sources/` — per-document summaries created by `/kb-ingest`
   - `knowledge/entities/` — people, orgs, products, and other proper nouns
   If the daily log mentions a person, company, product, or source document that **already**
   has an entity or source page, link to it via `[[entities/slug]]` or `[[sources/slug]]`
   and add the new fact to that existing page instead of creating a duplicate concept.
   Do NOT create new entity or source pages from a conversation — those belong to `/kb-ingest`.
4. **Create connection articles** in `knowledge/connections/` if this log reveals non-obvious
   relationships between 2+ existing concepts, entities, or sources
...
- Read-only (populated by /kb-ingest): {SOURCES_DIR}, {ENTITIES_DIR}
