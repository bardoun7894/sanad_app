// User-only entrypoint — for Play Store build (no admin screens).
// Build with:
//   flutter build appbundle -t lib/main_user.dart
//   flutter build apk -t lib/main_user.dart
//
// The full app (with admin) uses lib/main.dart.
...
void main() async {
  ...
  if (!kIsWeb) {
    ZegoUIKitPrebuiltCallInvitationService().setNavigatorKey(userNavigatorKey);
  ...
  runApp(const _UserStartupBootstrapApp());
}

class _UserStartupBootstrapApp extends StatefulWidget {
  const _UserStartupBootstrapApp();

  @override
  State<_UserStartupBootstrapApp> createState() =>
      _UserStartupBootstrapAppState();
}

class _UserStartupBootstrapAppState extends State<_UserStartupBootstrapApp> {
    ...
    unawaited(
      _runStep(
        ...
        action: () async {
          ...
          fcmService.setNavigatorKey(userNavigatorKey);
  ...
  Future<void> _runStep({
    ...
    try {
      ...
      sw.stop();
      debugPrint(
          '❌ Startup step failed: $label (${sw.elapsedMilliseconds}ms)');
      debugPrint('   reason: $e');
